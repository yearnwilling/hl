...
class FileToolkit
{
     ...
     * @return array|string
     */
    public static function pathinfo($filepath, $type = 0)
    {
        $pathinfo = array();
        $pathinfo['dirname'] = rtrim(substr($filepath, 0, strrpos($filepath, '/')), "/") . "/";
        $pathinfo['basename'] = ltrim(substr($filepath, strrpos($filepath, '/')), "/");
        $pathinfo['extension'] = substr(strrchr($filepath, '.'), 1);
        $pathinfo['filename'] = ltrim(substr($pathinfo['basename'], 0, strrpos($pathinfo['basename'], '.')), "/");
        switch ($type) {
    ...
    public static function validateFileMaxSize(File $file, $maxSize)
    {
        ...
        if ($file->getSize() > $maxSize) {
            $errors[] = "上传的文件不能大于：".static::formatFileSize($maxSize);
        }
    ...
    public static function remove($filepath)
    {
        ...
        if (!$isRemoved) {
            ...
            throw new \RuntimeException("The file located in `{$filepath}` is not allowed to be deleted without prefix {$prefixString}");
        }
    }
